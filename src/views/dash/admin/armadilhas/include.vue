    ...
    <div v-else class="form">
      ...
      <div v-if="rota == 'especie'">
        ...
        <select v-model="includeEspecie.tipo">
          ...
          <option
            v-for="(item, index) in armadilhasTipo"
            :key="index"
            :value="item.chave"
      ...
      <div v-if="rota == 'estagio'" class="listaEstagio">
        ...
        <div class="scope">
          ...
          <div class="include">
            <select v-model="includeEstagio.especie">
              <option value="selecione">selecione</option>
              <option
                v-for="(item, index) in armadilhasEspecie"
                :key="index"
                :value="item.chave"
                >{{ item.dados.name }}</option
              >
        ...
        <div class="scope">
          ...
          <div
            v-for="(estagio, index) in armadilhasEstagio.filter(
              item => includeEstagio.especie == item.dados.especie
            )"
            :key="index"
            class="item"
...
export default {
  ...
  methods: {
    ...
    sendPrioritaryImage(referenciaURL, chave) {
      servArmadilhasEspecie.child(chave).update({ imagemURL: referenciaURL });
    },
    uploadImagem() {
      const self = this;
      const chave = this.includeEspecie.chave;
      Storage.child(`armadilhas/${chave}/armadilha.jpg`)
    ...
    createTipo() {
      this.includeTipo.chave = servArmadilhasTipo.push().key;
      servArmadilhasTipo
    ...
    createEspecie() {
      ...
      this.includeEspecie.chave = servArmadilhasEspecie.push().key; //get key push
      //this.uploadImagem();
      servArmadilhasEspecie
    ...
    createEstagio() {
      ...
      this.includeEstagio.chave = servArmadilhasEstagio.push().key;
      servArmadilhasEstagio
