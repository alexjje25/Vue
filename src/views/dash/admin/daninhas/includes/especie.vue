<template>
  <div class="layout">
    ...
    <div class="lista">
      <h3>
        <v-badge color="green" :content="GET_DANINHAS_ESPECIE.length"
      ...
      <div
        class="item"
        :key="index"
        v-for="(praga, index) in GET_DANINHAS_ESPECIE"
      >
        {{ praga.dados.name }}
...
<script>
import { servDaninhasEspecie } from "@/database";
export default {
  data() {
    return {
      include: {
  ...
  methods: {
    REMOVE(chave) {
      ...
      if (decisao) {
        servDaninhasEspecie.child(chave).remove();
        window.alert("Removido com sucesso");
      } else {
        window.alert("Cancelado com sucesso");
      }
      //let lista = this.$store.getters["admin/GET_DANINHAS_ESPECIE"];
    ...
    POST() {
      let POST_KEY = servDaninhasEspecie.push().key;
      let self = this;
