<template>
  <div class="layout">
      ...
      <div>
        ...
        <select v-model="include.tipo">
          ...
          <option
            :key="index"
            :value="item.chave"
            v-for="(item, index) in GET_DANINHAS_TIPO"
      ...
      <div>
        ...
        <select v-model="include.especie">
          ...
          <option
            :key="index"
            :value="item.chave"
            v-for="(item, index) in GET_DANINHAS_ESPECIE"
            >{{ item.dados.name }}</option
          >
        </select>
      </div>
    ...
    <div class="lista">
      <h3>
        <v-badge color="green" :content="GET_DANINHAS_ESTAGIO.length"
      ...
      <div
        class="item"
        :key="index"
        v-for="(item, index) in GET_DANINHAS_ESTAGIO"
      >
...
<script>
import { servDaninhasEstagio } from "@/database";
...
export default {
  ...
  methods: {
    ...
    updateEstagio(estagio, tipo) {
      servDaninhasEstagio.child(estagio).update({
        tipo: tipo
      });
    },
    findEspecie(especie, estagio) {
      this.$store.getters["admin/GET_DANINHAS_ESPECIE"].find(item => {
    ...
    findErrorEstagio() {
      this.$store.getters["admin/GET_DANINHAS_ESTAGIO"].filter(item => {
        if (item.dados.tipo) {
    ...
    POST() {
      ...
      if (
        ...
        let POST_KEY = servDaninhasEstagio.push().key;
        let self = this;
  ...
  computed: {
    ...
    GET_DANINHAS_ESPECIE() {
      return this.$store.getters["admin/GET_DANINHAS_ESPECIE"].filter(
    ...
    GET_DANINHAS_ESTAGIO() {
      return this.$store.getters["admin/GET_DANINHAS_ESTAGIO"].filter(item => {
        if (
