<template>
  <div class="layout">
      ...
      <div>
        ...
        <select v-model="include.especie">
          ...
          <option
            :key="index"
            :value="item.chave"
            v-for="(item, index) in GET_PRAGAS_ESPECIE"
            >{{ item.dados.name }}</option
          >
        </select>
      </div>
    ...
    <div class="lista">
      <h3>
        <v-badge color="green" :content="GET_PRAGAS_ESTAGIO.length"
      ...
      <div
        class="item"
        :key="index"
        v-for="(item, index) in GET_PRAGAS_ESTAGIO"
      >
...
<script>
import { servPragasEstagio } from "@/database";
...
export default {
  ...
  methods: {
    ...
    updateEstagio(estagio, tipo) {
      servPragasEstagio.child(estagio).update({
        tipo: tipo
      });
    },
    findEspecie(especie, estagio) {
      this.$store.getters["admin/GET_PRAGAS_ESPECIE"].find(item => {
    ...
    findErrorEstagio() {
      this.$store.getters["admin/GET_PRAGAS_ESTAGIO"].filter(item => {
        if (item.dados.tipo) {
    ...
    REMOVE(chave) {
      ...
      if (decisao) {
        servPragasEstagio.child(chave).remove();
    ...
    POST() {
      ...
      if (
        ...
        let POST_KEY = servPragasEstagio.push().key;
        let self = this;
  ...
  computed: {
    ...
    GET_PRAGAS_ESTAGIO() {
      return this.$store.getters["admin/GET_PRAGAS_ESTAGIO"].filter(item => {
        if (
