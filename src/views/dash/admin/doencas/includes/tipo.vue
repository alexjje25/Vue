<template>
  <div class="layout">
    ...
    <div class="lista">
      <h3>
        <v-badge color="green" :content="GET_DOENCAS_TIPO.length"
          >Lista de Pragas Cadastradas</v-badge
        >
      </h3>
      <div class="item" :key="index" v-for="(praga, index) in GET_DOENCAS_TIPO">
        {{ praga.dados.name }}
...
<script>
import { servDoencasTipo } from "@/database";
export default {
  data() {
    return {
      include: {
  ...
  methods: {
    ...
    POST() {
      let POST_KEY = servDoencasTipo.push().key;
      let self = this;
