  ...
  computed: {
    ...
    GET_ESTOQUE() {
      return this.$store.getters["fazenda/GET_ESTOQUE"].filter(item => {
        if (item.dados.modulo == "pecas") {
          if (item.dados.categoria == "veiculos") {
            return true;
          } else {
            return false;
          }
    ...
    enviar() {
      ...
      if (this.include.colaborador == "selecione") {
        this.$store.commit("resource/SET_NOTIFY", {
          type: "warning",
          status: true,
          mensagem: "Não foi selecionado colaborador"
        });
        return false;
      }
      if (this.include.quantidade <= 0) {
      ...
      if (this.include.tipoOperacao == "entrada") {
        if (
          this.include.dataEntrega == "" ||
          this.include.dataEntrega == undefined
        ) {
          this.$store.commit("resource/SET_NOTIFY", {
            type: "warning",
            status: true,
            mensagem:
              "Falta selecionar a data de entrega, caso o produto já esteja em estoque, coloque a data atual ou 1 dia anterior"
          });
          return false;
        } else {
          this.enviarImagem();
