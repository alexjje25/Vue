<template>
  <div class="containReports">
    ...
    <div class="watchPragas">
            ...
            <div>
              ...
              <input v-model="search.dataPrevista" type="date" />
                  ...
                  <bar-chart class="barchart"
                      v-if="showChart == true"
                      :chart-data="this.myDataPragas20"
      ...
      <v-view-map class="mapa" :lista="pragas" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
   FILTRAR_PRAGAS_VINTE_DIAS(databin) {
    ...
    GERAR_TABELAS_CHART_PRAGAS_VINTE_DIAS(b) {
      if (b == 1) {
         let i = 20;
         let arrayToChart = [];
          for (i; i >= 0; i--) {
            ...
            this.myDataPragas20.labels.push(databin)

            let pragasAtivadas20dias = this.FILTRAR_PRAGAS_VINTE_DIAS(databin);
            
            let valt = pragasAtivadas20dias.length;
            arrayToChart.push(valt);
            if (i == 1) {
              this.carregou = 1;
              this.myDataPragas20.datasets[0].data = arrayToChart;
              this.carregou = 2;
              this.showChart = true;
            }
         }
  ...
  beforeMount() {
     ...
     this.GERAR_TABELAS_CHART_PRAGAS_VINTE_DIAS(1);
  }
};
</script>
