<template>
  <div class="containReports">
    ...
    />
    <div class="watchDoencas">
      <!-- doencas -->
      <!-- old
      <div class="contentBOX">
        <div class="tituloContent">Monitoramento de doencas</div>
        <div class="buscador g6" :data-count="doencas.length">
          <div>
            PRODUÇÃO:<br />
          ...
          <div>
            <div class="pragaCount">
              RESULTADOS
              <div class="sizeUP">{{ doencas.length }}</div>
            </div>
          </div>
          <div>
            <img
      ...
      <v-view-map class="mapa" :lista="doencas" />-->
      <div class="contentBOX">
        <!-- mark 12052020 -->
        <div class="boxPadrao">
          <div class="contain2 centralizaTudo">
            <vMonitor :lista="doencas" />
          ...
          <div class="searchs g4 centralizaTudo">
            ...
            <div>
              ...
              <bar-chart class="barchart"
                      v-if="showChart == true"
                      :chart-data="this.myDataDoencas20"
...
export default {
  ...
  data() {
    return {
      ...
      search: {
        ...
        datasets: [
          {
            label: "Doença - 20 Dias",
            backgroundColor: "#e2e030",
            data: [],
            borderColor: "#8e851d",
            borderWidth: 2
          },
  ...
  computed: {
    ...
    doencas() {
      let self = this;
      let response1 = this.$store.getters["fazenda/GET_DOENCAS"].filter(
        item => {
  ...
  methods: {
     FILTRAR_DOENCA_VINTE_DIAS(databin) {
   ...
   GERAR_TABELAS_CHART_DOENCAS_VINTE_DIAS (b) {
      if (b == 1) {
         let i = 20;
         let arrayToChart = [];
          for (i; i >= 0; i--) {
            ...
            this.myDataDoencas20.labels.push(databin)

            let doencaAtivadas20dias = this.FILTRAR_DOENCA_VINTE_DIAS(databin);
            
            let valt = doencaAtivadas20dias.length;
            arrayToChart.push(valt);
            if (i == 1) {
              this.carregou = 1;
              this.myDataDoencas20.datasets[0].data = arrayToChart;
              this.carregou = 2;
              this.showChart = true;
            }
         }
    ...
    sendImpress() {
      ...
      this.$store.commit("SET_IMPRESS_LIST", this.doencas);
      this.$store.commit("SET_IMPRESS_DATABIN", pay);
      this.$store.commit("SET_IMPRESS_TYPE", "relatorio-doencas");
  ...
  beforeMount() {
      ...
      this.GERAR_TABELAS_CHART_DOENCAS_VINTE_DIAS(1);
  }
};
</script>
